...
import '../widgets/sunflower_button.dart';
...
class _HomeScreenState extends ConsumerState<HomeScreen>
  ...
  Widget build(BuildContext context) {
    ...
    final clockCustomization = ref.watch(clockCustomizationProvider).value ?? const ClockCustomization();
    final neoSettings = ref.watch(homeScreenNeoProvider).value ?? const HomeScreenNeoSettings();

    Widget mainContent = DragTarget<AppInfo>(
          ...
          children: [
            ...
            appsAsyncValue.when(
              data: (allApps) {
                ...
                final List<AppInfo> allFloatingApps = List.from(homeAppsList);

                return Stack(
                  fit: StackFit.expand,
                  children: [
                    // Draggable Custom Clock
                    if (clockCustomization.showClock)
                      Positioned(
                ...
                children: [
                    ...
                    child: Container(
                      ...
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          ...
                          prefixIcon: Icon(Icons.search, color: Colors.white70, size: 20.sw(context)),
                          border: InputBorder.none,
                          contentPadding: EdgeInsets.symmetric(vertical: 10.sh(context)),
                          suffixIcon: _isSearching
                              ? IconButton(
                                  icon: Icon(Icons.close, color: Colors.white70, size: 20.sw(context)),
                                  onPressed: () {
                                    setState(() {
                                      _isSearching = false;
                                      _searchQuery = "";
                  ...
                  GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        color: isEditMode ? Colors.white.withAlpha(50) : Colors.white.withAlpha(10),
                        shape: BoxShape.circle,
                      ),
                      child: Icon(isEditMode ? Icons.done : Icons.edit, color: Colors.white, size: 23.sh(context)),
                    ),
                  ),
                  if (isAddButtonVisible) ...[
                    ...
                    GestureDetector(
                      ...
                      child: Container(
                        ...
                        child: Icon(Icons.add, color: Colors.white, size: 30.sh(context)),
                      ),
                    ),
                  ],
                ],
            ...
            Positioned(
              ...
              right: 10.sw(context),
              child: IconButton(
                icon: Icon(Icons.settings, color: Colors.white, size: 30.sw(context)),
                onPressed: () {
                  FocusScope.of(context).unfocus();
                          ...
                          Expanded(
                            child: Consumer(
                              builder: (context, ref, _) {
                                final allApps = ref.watch(appsProvider).value ?? [];
                                final hiddenApps = ref.watch(hiddenAppsProvider).value ?? {};
                                final results = allApps
                                    .where((app) =>
                                        app.label.toLowerCase().contains(_searchQuery) &&
                                  ...
                                  itemBuilder: (context, index) {
                                    ...
                                    return ListTile(
                                      leading: SizedBox(
                                          width: 40,
                                          height: 40,
                                          child: AppIconContent(app: app, showLabel: false, size: 40)),
